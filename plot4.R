## filename: plot4.R
## purpose: to build four differnt plots in one file/image as per
...
## into a bunch of variables x (which will always be the x-axis) and one or
## more variables for each add'l chart.  This use of variables makes easier 
## adjustment if/when I have to work this code.
x<-power_data$EventDateTime
sm1<-power_data$Sub_metering_1
sm2<-power_data$Sub_metering_2
sm3<-power_data$Sub_metering_3
chart1<-power_data$Global_active_power
chart2<-power_data$Voltage
chart4<-power_data$Global_reactive_power

## first open the image file/graphics device
png(filename="plot4.png",width=480,heigh=480,units="px")
## now set that device to have 4 plots, in a 2/2 grid
par(mfrow=c(2,2))

## now make each of the plots, in turn
plot(x,chart1,type="l",xlab="",ylab="Global Active Power")
plot(x,chart2,type="l",xlab="",ylab="Voltage")
plot(x,sm1,type="l",xlab="",ylab="Energy sub metering")
lines(x,sm2,type="l",col="Red")
lines(x,sm3,type="l",col="Blue")
legend("topright",legend=c("Sub_Metering_1","Sub_Metering_2","Sub_Metering_3"),col=c("Black","Red","Blue"),lty=c(1,1,1))
plot(x,chart4,type="l",xlab="datetime",ylab="global_reactive_power")
